...
async function registerController(req, res) {
  try {
    ...
    if (!fName || !lName || !gender || !roleId || !email || !password) {
      return res.status(200).json({ statusCode: 200, taskStatus: false, message: 'ไม่พบข้อมูล' });
    }

    const [existingUser] = await db.promise().query('SELECT * FROM users WHERE email = ? ', [email]);
    if (existingUser.length > 0) {
      return res.status(200).json({ statusCode: 200, taskStatus: false, message: 'อีเมลนี้ถูกใช้งานแล้ว' });
    }

    const hash_password = await bcrypt.hash(password, 10);

    if (!hash_password) {
      return res.status(200).json({ statusCode: 200, taskStatus: false, message: 'ไม่สามารถใช้รหัสผ่านนี้ได้' });
    } else {
      let profile = null;
      let fileName = null;
      ...
      const query = 'INSERT INTO users (fName, lName, gender, roleId, email, password, usrImg) VALUES (?, ?, ?, ?, ?, ?, ?)';
      const values = [fName, lName, gender, roleId, email, hash_password, profile];
      ...
      if (result && file && profile) {
        ...
        if (!upload_state) {
          return res.status(200).json({ statusCode: 200, taskStatus: false, message: 'ไม่สามารถอัพโหลดไฟล์ภาพได้' });
        }
